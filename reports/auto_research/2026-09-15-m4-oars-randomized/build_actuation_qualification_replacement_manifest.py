...
"""Build the single authorized enacted-OARS qualification replacement."""
...
AUTHORIZATION_SHA256 = (
    "78f89bf5ed94bd9a905d48a2a4e282c79bc9c7de91adf3bdda0ebd9e47b36e7b"
)
AMENDMENT_SHA256 = "38a23ae7829ef328013f9bac90c74859eb36c7d7ddf16be236f7be76e669637a"
...
NAME = "m4-oars-qualification-act-replacement"
...
def main() -> None:
    ...
    for path, expected in (
        ...
        if sha256(path) != expected:
            raise RuntimeError(f"frozen replacement input moved: {path.name}")
    authorization = json.loads(args.authorization.read_bytes())
    amendment = json.loads(args.amendment.read_bytes())
    if (
        authorization["schema"]
        != "m4-oars-actuation-qualification-replacement-authorization-v1"
        ...
        or authorization["replacement_attempt_limit"] != 1
    ...
    if (
        amendment["schema"]
        != "m4-oars-actuation-qualification-replacement-amendment-v1"
        or amendment["status"] != "FROZEN_BEFORE_REPLACEMENT_SUBMISSION"
        or amendment["repair_provenance"]["replacement_source_commit"]
        != args.source_commit
        or amendment["repair_provenance"]["transferqueue_full_commit"]
        != TRANSFERQUEUE_COMMIT
        or amendment["allowed_change"]["actor_runtime_env_mode"]
        != "inherit_baked_single_node"
        or amendment["scientific_outcome_acquisition"]
    ):
        raise RuntimeError("replacement amendment differs")
    ...
    if not required_source.issubset(names):
        raise RuntimeError("replacement source archive lacks runtime files")
...
print("M4_OARS_REPLACEMENT_SAFE_SOURCE_PASS",len(names),len(symlinks))
PY
...
print("M4_OARS_REPLACEMENT_MEGATRON_PASS")
PY
cd "$RUN_REPO"
...
assert m["status"]=="FROZEN_BEFORE_REPLACEMENT_SUBMISSION"
...
print("M4_OARS_REPLACEMENT_RUNTIME_AUTHORITY_CONFIG_AND_TQ_PASS")
PY
