...
readonly SBATCH_ACCOUNT="nemotron_edge_omni"
readonly SBATCH_PARTITION="interactive"
...
export NRL_RUN_PREFIX="rl_main_vg2_sync_video_unlimited"
...
uv run --project /opt/nemo-rl --no-sync python \
  ...
  --config examples/nemo_gym/grpo_nemotron_omni_30ba3b_video_sync.yaml \
...
sbatch \
  ...
  --job-name=rl_main_vg2_sync_video \
  --output="${NEMO_RL_RUN_ROOT}/slurm-2n-sync-%j.out" \
  ray.sub
