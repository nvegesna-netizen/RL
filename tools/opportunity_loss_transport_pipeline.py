...
"""Detached analysis pipeline for the Qwen3-1.7B M4 transport study."""
...
def _validate_transport_contract(
    ...
    if options["protocol_identity"] != "m4-opportunity-loss-qwen3-1p7b-transport-v1":
        raise OpportunityLossPipelineError("transport pipeline requires Qwen3-1.7B v1")
    if tuple((arm.label, arm.delay_seconds, arm.mass) for arm in protocol.arms) != (
        ("control", 0.0, 1),
        ("d5", 5.0, 1),
    ):
        raise OpportunityLossPipelineError("transport arms disagree")
    if (
        protocol.assignment_domain != "m4-opportunity-loss-qwen3-1p7b-transport-v1"
        or protocol.assignment_seed != 20260907
        or protocol.primary_start_version != 8
        or protocol.primary_end_version != 507
    ):
        raise OpportunityLossPipelineError("transport assignment or window disagrees")
    if (
        options.get("cross_fit_folds") != 8
        or options.get("hac_lag") != 4
        or options.get("block_size") != 8
        or options.get("bootstrap_draws") != 20000
        or options.get("bootstrap_seed") != 20260908
        or options.get("max_staleness_versions") != 1
    ):
        raise OpportunityLossPipelineError("transport inference geometry disagrees")
    if raw.get("windows") != {
        "burn_in_start_versions": [0, 7],
        "primary_start_versions": [8, 507],
        "terminal_guard_start_versions": [508, 557],
    }:
        raise OpportunityLossPipelineError("transport windows disagree")
    if raw.get("runtime") != {
        "buffer_capacity": 64,
        ...
        raise OpportunityLossPipelineError("transport runtime disagrees")
    if raw.get("resource_caps") != {
        ...
        raise OpportunityLossPipelineError("transport resource caps disagree")
...
def build_transport_result(
    *,
    ...
    """Strictly reconstruct and analyze one Qwen3-1.7B transport bundle."""
    ...
    return {
        "schema_version": 1,
        "result_scope": "qwen3_1p7b_transport_adjusted_primary_and_supporting_checks",
